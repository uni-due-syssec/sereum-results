...
def unknown474dacce(addr m_param1, bool m_param2): # not payable
  if munknown89977cdfAddress != caller:
      require bool(mstor2m[callerm]) == 1
...
def AddMessage(address m_adr, uint256 m_val, string m_data): # not payable
  ...
  if not mstor3m[addr(m_adr)m]:
      mstor5 = m_adr
      mstor8 = block.timestamp
      mstor7 = m_val
      uint256(mstor6m[m]) = Array(len=m_data.length, data=m_data[allm])
      mhistorym.length++
      if not mhistorym.length <= mhistorym.length + 1:
          mem[0] = 4
          [94midx = 4 * mhistorym.length + 1
          mwhile sha3(4) + (4 * mhistorym.length) > [94midx + sha3(mem[0])m:
              ...
              [94midx = [94midx + 4
              mcontinue 
      addr(mhistorym[mhistorym.lengthm]m.field_0) = mstor5
      if 31 >= mstor6m.length:
          uint256(mhistorym[mhistorym.lengthm]m.field_256) = mstor6m.length
          [94midx = 0
          mwhile mstor[(4 * mhistorym.length) + ('name', 'history', 4) + 1m]m.length + 31 / 32 > [94midxm:
              uint256(mstor[[94midx + sha3((4 * mhistorym.length) + ('name', 'history', 4) + 1)m]m.field_0) = 0
              [94midx = [94midx + 1
              mcontinue 
      else:
          uint256(mhistorym[mhistorym.lengthm]m.field_256) = Mask(255, 1, (256 * not bool(mstor6m.length)) - 1 and mstor6m.length) + 1
          if not Mask(255, 1, (256 * not bool(mstor6m.length)) - 1 and mstor6m.length):
              [94midx = 0
              mwhile mstor[(4 * mhistorym.length) + ('name', 'history', 4) + 1m]m.length + 31 / 32 > [94midxm:
                  uint256(mstor[[94midx + sha3((4 * mhistorym.length) + ('name', 'history', 4) + 1)m]m.field_0) = 0
                  [94midx = [94midx + 1
              ...
              [94ms = 0
              [94midx = 0
              mwhile mstor6m.length + 31 / 32 > [94midxm:
                  uint256(mstor[[94ms + sha3((4 * mhistorym.length) + ('name', 'history', 4) + 1)m]m.field_0) = uint256(mstor6m[[94midxm])
                  [94ms = [94ms + 1
                  [94midx = [94midx + 1
                  mcontinue 
              [94midx = mstor6m.length + 31 / 32
              mwhile mstor[(4 * mhistorym.length) + ('name', 'history', 4) + 1m]m.length + 31 / 32 > [94midxm:
                  uint256(mstor[[94midx + sha3((4 * mhistorym.length) + ('name', 'history', 4) + 1)m]m.field_0) = 0
                  [94midx = [94midx + 1
                  mcontinue 
      uint256(mhistorym[mhistorym.lengthm]m.field_512) = mstor7
      uint256(mhistorym[mhistorym.lengthm]m.field_768) = mstor8
...
def History(uint256 m_param1): # not payable
  ...
  mem[256] = uint256(mstor[sha3((4 * m_param1) + ('name', 'history', 4) + 1)m]m.field_0)
  [94midx = 256
  [94ms = 0
  mwhile mstor[(4 * m_param1) + ('name', 'history', 4) + 1m]m.length + 256 > [94midx + 32m:
      mem[[94midx + 32] = uint256(mstor[[94ms + sha3((4 * m_param1) + ('name', 'history', 4) + 1)m]m.field_256)
      [94midx = [94midx + 32
      [94ms = [94ms + 1
      mcontinue 
  return addr(mhistorym[m_param1m]m.field_0), 
         Array(len=mstor[(4 * m_param1) + ('name', 'history', 4) + 1m]m.length, data=mem[256 len mstor[(4 * m_param1) + ('name', 'history', 4) + 1m]m.length + (floor32(mstor[(4 * m_param1) + ('name', 'history', 4) + 1m]m.length - 1) + -mstor[(4 * m_param1) + ('name', 'history', 4) + 1m]m.length + 32 % 32)]),
