...
#  addr = 0xDBdA6655f1dA6A4Cf9513e02A112051d0152fbe7 
#  network = mainnet 
#  ver = 26 Apr 2019 
#  problems = {} 
...
def unknown474dacce(addr _param1, bool _param2): # not payable
  ...
  stor1[addr(_param1)] = uint8(_param2)
...
def AddMessage(address _adr, uint256 _val, string _data): # not payable
  ...
  if not stor1[addr(_adr)]:
      stor3 = _adr
      stor6 = block.timestamp
      stor5 = _val
      uint256(stor4[]) = Array(len=_data.length, data=_data[all])
      history.length++
      ...
      if 31 >= stor4.length:
          ...
          if not Mask(255, 1, (256 * not bool(stor4.length)) - 1 and stor4.length):
              ...
              while stor4.length + 31 / 32 > [94midx:
                  uint256(stor[[94ms + sha3((4 * history.length) + ('name', 'history', 2) + 1)].field_0) = uint256(stor4[[94midx])
                  [94ms = [94ms + 1
                  [94midx = [94midx + 1
                  continue 
