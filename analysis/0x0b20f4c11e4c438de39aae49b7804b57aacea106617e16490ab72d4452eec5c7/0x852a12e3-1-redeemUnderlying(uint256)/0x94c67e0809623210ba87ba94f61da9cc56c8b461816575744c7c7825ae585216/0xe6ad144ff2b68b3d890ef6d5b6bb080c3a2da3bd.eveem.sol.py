...
# const = None
# payable = False
def _setPendingAdmin(address _newPendingAdmin): # not payable
...
def unknownc1e80334(): # not payable
  if unknowndcfbc0c7Address != caller:
      log Failure(
            uint256 error=1,
            uint256 info=1,
            uint256 detail=0)
      return 1
  if not unknowndcfbc0c7Address:
      log Failure(
            uint256 error=1,
            uint256 info=1,
            uint256 detail=0)
      return 1
  unknownbb82aa5eAddress = unknowndcfbc0c7Address
  unknowndcfbc0c7Address = 0
...
# const = None
# payable = False
def unknowne992a041(addr _param1): # not payable
  require calldata.size - 4 >= 32
  if adminAddress != caller:
      log Failure(
            uint256 error=1,
            ...
            uint256 detail=0)
      return 1
  unknowndcfbc0c7Address = _param1
...
def _fallback() payable: # default function
  [93mdelegate unknownbb82aa5eAddress with:
