...
#  addr = 0x3a91A1108506B90b3A7347dB4A670d145F95D3DC 
#  network = mainnet 
#  ver = 26 Apr 2019 
#  problems = {} 
...
# const = None
# payable = False
def unknown21a244e5(addr _param1): # not payable
  return unknown21a244e5[_param1]
...
def unknowna6c30b29(): # not payable
  require not stor14[caller]
  require eth.balance(caller) > 0
  unknown21a244e5[caller] = gas_remaining
  stor14[caller] = 1
...
def unknownd3927c15(): # not payable
  require stor14[caller]
  require unknown21a244e5[caller] > 0
  require gas_remaining >= 10000
  require eth.balance(caller) > 0
  require gas_remaining <= unknown21a244e5[caller]
  if not unknown21a244e5[caller] + -gas_remaining + 21000:
      require 0 <= eth.balance(caller)
      stor14[caller] = 0
      unknown21a244e5[caller] = 0
      return 0
  require (21000 * block.gasprice) + (unknown21a244e5[caller] * block.gasprice) - (gas_remaining * block.gasprice) / unknown21a244e5[caller] + -gas_remaining + 21000 == block.gasprice
  require (21000 * block.gasprice) + (unknown21a244e5[caller] * block.gasprice) - (gas_remaining * block.gasprice) <= eth.balance(caller)
  stor14[caller] = 0
  unknown21a244e5[caller] = 0
  return ((21000 * block.gasprice) + (unknown21a244e5[caller] * block.gasprice) - (gas_remaining * block.gasprice))
...
# const = None
# payable = False
def lock(address _owner): # not payable
  return bool(stor14[_owner])
