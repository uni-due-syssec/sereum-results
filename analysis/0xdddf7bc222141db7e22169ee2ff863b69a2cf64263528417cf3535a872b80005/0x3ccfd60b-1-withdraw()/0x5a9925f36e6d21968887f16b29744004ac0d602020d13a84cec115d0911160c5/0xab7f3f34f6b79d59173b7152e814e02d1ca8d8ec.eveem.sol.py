...
#  addr = 0xAb7F3f34F6b79D59173b7152E814e02D1cA8D8Ec 
#  network = mainnet 
#  ver = 26 Apr 2019 
#  problems = {} 
...
# const = None
# payable = False
def getPrices(address[] m_ofAssets): # not payable
  if not m_ofAssets.length:
      mem[(32 * m_ofAssets.length) + 128] = m_ofAssets.length
      [94midx = 0
      mwhile [94midx < m_ofAssets.lengthm:
          [94mt = 0
          [94mt = 0
          [94ms = 0
          mwhile [94ms < mexchangesm.lengthm:
              mem[0] = mexchangesm[[94msm]m.field_0
              mem[32] = 1
              require ext_code.size(mexchangeAdaptersm[mstor2m[[94msm]m.field_0m])
              call mexchangeAdaptersm[mstor2m[[94msm]m.field_0m].isEnabled() with:
                   gas gas_remaining wei
              mem[(64 * m_ofAssets.length) + 160] = ext_call.return_data[0]
              if not ext_call.success:
                  revert with ext_call.return_data[0 len return_data.size]
              else:
                  require return_data.size >= 32
                  if ext_call.return_data[0]:
                      mem[(64 * m_ofAssets.length) + 196] = addr(cd[((32 * [94midx) + m_ofAssets + 36)])
                      mem[(64 * m_ofAssets.length) + 228] = 10^18
                      require ext_code.size(mexchangeAdaptersm[mstor2m[[94msm]m.field_0m])
                      call mexchangeAdaptersm[mstor2m[[94msm]m.field_0m].getPrice(address sourceAddress, address destAddress, uint256 amount) with:
                           gas gas_remaining wei
                          args 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee, addr(cd[((32 * [94midx) + m_ofAssets + 36)]), 10^18
                      mem[(64 * m_ofAssets.length) + 160 len 64] = ext_call.return_data[0 len 64]
                      if not ext_call.success:
                          revert with ext_call.return_data[0 len return_data.size]
                      else:
                          require return_data.size >= 64
                          if ext_call.return_data[0]:
                              if ext_call.return_data[32] >=′ 0:
                                  if ext_call.return_data[32] <′ -1:
                                      [94mt = mexchangeAdaptersm[mstor2m[[94msm]m.field_0m]
                                      [94mt = mexchangesm[[94msm]m.field_0
                                      [94ms = [94ms + 1
                                      mcontinue 
                                  else:
                                      [94mt = mexchangeAdaptersm[mstor2m[[94msm]m.field_0m]
          ...
          [94midx = [94midx + 1
          mcontinue 
      mem[(64 * m_ofAssets.length) + 160] = 64
      mem[(64 * m_ofAssets.length) + 224] = m_ofAssets.length
      mem[(64 * m_ofAssets.length) + 256 len floor32(m_ofAssets.length)] = mem[128 len floor32(m_ofAssets.length)]
  else:
      mem[128 len 32 * m_ofAssets.length] = code.data[4702 len 32 * m_ofAssets.length]
      mem[(32 * m_ofAssets.length) + 128] = m_ofAssets.length
      mem[(32 * m_ofAssets.length) + 160 len 32 * m_ofAssets.length] = code.data[4702 len 32 * m_ofAssets.length]
      [94midx = 0
      mwhile [94midx < m_ofAssets.lengthm:
          [94mt = 0
          [94mt = 0
      ...
      mem[(64 * m_ofAssets.length) + 160] = 64
      mem[(64 * m_ofAssets.length) + 224] = m_ofAssets.length
      mem[(64 * m_ofAssets.length) + 256 len floor32(m_ofAssets.length)] = code.data[4702 len floor32(m_ofAssets.length)]
  mem[(64 * m_ofAssets.length) + 192] = (32 * m_ofAssets.length) + 96
  mem[(98 * m_ofAssets.length) + 256] = mem[(32 * m_ofAssets.length) + 128]
  mem[(98 * m_ofAssets.length) + 288 len floor32(mem[(32 * m_ofAssets.length) + 128])] = mem[(32 * m_ofAssets.length) + 160 len floor32(mem[(32 * m_ofAssets.length) + 128])]
  return memory
    from (64 * m_ofAssets.length) + 160
     [93mlen (32 * mem[(32 * m_ofAssets.length) + 128]) + (32 * m_ofAssets.length) + 128
...
# const = None
# payable = False
def unknown9ddf8831(addr m_param1, addr m_param2, uint256 m_param3, uint256 m_param4): # not payable
  [94ms = 0
  [94ms = 0
  [94midx = 0
  mwhile [94midx < mexchangesm.lengthm:
