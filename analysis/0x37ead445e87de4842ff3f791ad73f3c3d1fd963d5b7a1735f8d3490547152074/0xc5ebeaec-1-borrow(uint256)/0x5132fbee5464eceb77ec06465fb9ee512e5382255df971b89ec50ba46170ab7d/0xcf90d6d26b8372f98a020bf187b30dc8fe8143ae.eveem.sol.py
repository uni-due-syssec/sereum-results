...
#  addr = 0xcF90D6d26b8372F98A020BF187b30dc8FE8143aE 
#  network = mainnet 
#  ver = 26 Apr 2019 
...
def unknown2e27f486(): # not payable
  mem[132 len 0] = None
  [93mdelegate mlogicContractAddress.mem[132 len 4] with:
...
# const = None
# payable = False
def unknownab7b1c89(uint256 m_param1): # not payable
  require calldata.size - 4 >= 32
  [93mdelegate mlogicContractAddress.0xab7b1c89 with:
       gas gas_remaining wei
      args Mask(224, 32, m_param1) >> 32, mem[196 len 4]
  require delegate.return_code
...
# const = None
# payable = False
def sellOrder(uint256 m_sellPrice, uint256 m_amount): # not payable
  require calldata.size - 4 >= 64
  mem[196 len 64] = sellOrder(uint256 sellPrice, uint256 amount), m_sellPrice, Mask(224, 32, m_amount) >> 32
  mem[288 len 4] = uint32(m_amount)
  [93mdelegate mlogicContractAddress with:
     funct uint32(m_sellPrice)
       gas gas_remaining wei
      args Mask(224, 32, m_amount) << 224, mem[260 len 4]
  if not return_data.size:
      require delegate.return_code
      return sellOrder(uint256 sellPrice, uint256 amount), m_sellPrice, Mask(224, 32, m_amount) >> 32
  mem[228 len return_data.size] = ext_call.return_data[0 len return_data.size]
  require delegate.return_code
  require return_data.size >= 64
  [94m_31 = mem[260 len 28], uint32(m_amount)
...
# const = None
# payable = False
def executeOrder(uint256 m_assetId, uint256 m_price): # not payable
  require calldata.size - 4 >= 64
  [93mdelegate mlogicContractAddress with:
     funct uint32(m_assetId)
       gas gas_remaining wei
      args Mask(224, 32, m_price) << 224, mem[260 len 4]
  require delegate.return_code
