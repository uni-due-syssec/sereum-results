...
#  addr = 0xD5D15cB78527E0eFd7C4D365Efb47C31CFEE3cce 
#  network = mainnet 
#  ver = 26 Apr 2019 
#  problems = {} 
...
# const = None
# payable = True
def unknown66325155(uint256 m_param1, uint256 m_param2, array m_param3) payable: 
  if call.value <= 0:
      call addr(mmultiAssetAddress).registryICAP() with:
           gas gas_remaining - 25050 wei
      if ext_call.success:
          ...
          if uint8(mstor4m[ext_call.return_data[12 len 20]m]):
              if caller == tx.origin:
                  ...
                  call addr(mmultiAssetAddress).transferFromToICAPWithReference(address from, bytes32 icap, uint256 value, string reference) with:
                       gas gas_remaining - 25050 wei
                      args caller, m_param1, m_param2, Array(len=m_param3.length, data=m_param3[allm])
              require ext_call.success
              return bool(ext_call.return_data[0])
          uint8(mstor3m.field_0) = 1
          if caller == tx.origin:
          ...
          if ext_call.success:
              ...
              call addr(munknownaa46f961Address).0xd882aa0 with:
                 value m_param2 wei
                   gas gas_remaining - 34050 wei
                  args m_param1, Array(len=m_param3.length, data=m_param3[allm])
              if ext_call.success:
                  if ext_call.return_data[0]:
                      return bool(ext_call.return_data[0])
  else:
