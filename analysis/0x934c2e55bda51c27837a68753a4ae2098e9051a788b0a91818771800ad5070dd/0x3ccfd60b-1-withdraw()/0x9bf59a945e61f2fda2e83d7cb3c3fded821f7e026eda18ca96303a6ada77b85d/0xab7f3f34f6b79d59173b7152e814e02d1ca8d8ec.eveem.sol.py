...
def getPrices(address[] _ofAssets): # not payable
  if not _ofAssets.length:
      ...
      while [94midx < _ofAssets.length:
          ...
          [94mt = 0
          [94ms = 0
          while [94ms < exchanges.length:
              mem[0] = exchanges[[94ms].field_0
              mem[32] = 1
              require ext_code.size(exchangeAdapters[stor2[[94ms].field_0])
              call exchangeAdapters[stor2[[94ms].field_0].isEnabled() with:
                   gas gas_remaining wei
              mem[(64 * _ofAssets.length) + 160] = ext_call.return_data[0]
              if not ext_call.success:
                  revert with ext_call.return_data[0 len return_data.size]
              else:
                  require return_data.size >= 32
                  if ext_call.return_data[0]:
                      mem[(64 * _ofAssets.length) + 196] = addr(cd[((32 * [94midx) + _ofAssets + 36)])
                      mem[(64 * _ofAssets.length) + 228] = 10^18
                      require ext_code.size(exchangeAdapters[stor2[[94ms].field_0])
                      call exchangeAdapters[stor2[[94ms].field_0].getPrice(address sourceAddress, address destAddress, uint256 amount) with:
                           gas gas_remaining wei
                          args 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee, addr(cd[((32 * [94midx) + _ofAssets + 36)]), 10^18
                      mem[(64 * _ofAssets.length) + 160 len 64] = ext_call.return_data[0 len 64]
                      if not ext_call.success:
                          revert with ext_call.return_data[0 len return_data.size]
                      else:
                          require return_data.size >= 64
                          if ext_call.return_data[0]:
                              if ext_call.return_data[32] >=′ 0:
                                  if ext_call.return_data[32] <′ -1:
                                      [94mt = exchangeAdapters[stor2[[94ms].field_0]
                                      [94mt = exchanges[[94ms].field_0
                                      [94ms = [94ms + 1
                                      continue 
                                  else:
                                      [94mt = exchangeAdapters[stor2[[94ms].field_0]
          ...
          [94midx = [94midx + 1
          continue 
      mem[(64 * _ofAssets.length) + 160] = 64
      mem[(64 * _ofAssets.length) + 224] = _ofAssets.length
      mem[(64 * _ofAssets.length) + 256 len floor32(_ofAssets.length)] = mem[128 len floor32(_ofAssets.length)]
  else:
      mem[128 len 32 * _ofAssets.length] = code.data[4702 len 32 * _ofAssets.length]
      mem[(32 * _ofAssets.length) + 128] = _ofAssets.length
      mem[(32 * _ofAssets.length) + 160 len 32 * _ofAssets.length] = code.data[4702 len 32 * _ofAssets.length]
      [94midx = 0
      while [94midx < _ofAssets.length:
          [94mt = 0
          [94mt = 0
      ...
      mem[(64 * _ofAssets.length) + 160] = 64
      mem[(64 * _ofAssets.length) + 224] = _ofAssets.length
      mem[(64 * _ofAssets.length) + 256 len floor32(_ofAssets.length)] = code.data[4702 len floor32(_ofAssets.length)]
  mem[(64 * _ofAssets.length) + 192] = (32 * _ofAssets.length) + 96
  ...
  return memory
    from (64 * _ofAssets.length) + 160
     [93mlen (32 * mem[(32 * _ofAssets.length) + 128]) + (32 * _ofAssets.length) + 128
...
# const = None
# payable = False
def unknown9ddf8831(addr _param1, addr _param2, uint256 _param3, uint256 _param4): # not payable
  [94ms = 0
  [94ms = 0
  [94midx = 0
  while [94midx < exchanges.length:
