...
#  addr = 0xbDB6590f46e2680bae783364739e5b3A7b3867dA 
#  network = mainnet 
#  ver = 26 Apr 2019 
...
# hash = 0x264be753
# getter = ['storage', 256, 0, ['sha3', ['data', ['mask_shl', 160, 0, 96, ['cd', 4]], 23]]]
# const = None
# payable = False
...
# hash = 0x3685083d
# getter = ['storage', 256, 0, ['sha3', ['data', ['cd', 36], ['sha3', ['data', ['cd', 4], 27]]]]]
# const = None
# payable = False
...
def unknown3c8bccd9(uint256 m_param1): # not payable
  ...
  if not m_param1:
      return 0
  require 2 * ext_call.return_data[0] * m_param1 / m_param1 == 2 * ext_call.return_data[0]
  return (2 * ext_call.return_data[0] * m_param1 / 100000)
...
def unknown440277e8(addr m_param1): # not payable
  ...
  mem[64] = 224
  require ext_code.size(addr(munknown264be753m[addr(m_param1)m]m[mstor26m[addr(m_param1)m]m]m.field_0))
  static call addr(munknown264be753m[addr(m_param1)m]m[mstor26m[addr(m_param1)m]m]m.field_0).0xe8967dbb with:
          gas gas_remaining wei
         args addr(m_param1), munknown264be753m[addr(m_param1)m]m[mstor26m[addr(m_param1)m]m]m.field_256
  if not ext_call.success:
      revert with ext_call.return_data[0 len return_data.size]
...
def unknownac53df0b(uint256 m_param1): # not payable
  ...
  if not ext_call.return_data[0]:
      if not m_param1:
          return 0
      require 100000 * m_param1 / m_param1 == 100000
      return (100000 * m_param1 / 100000)
  require 2 * ext_call.return_data[0] / ext_call.return_data[0] == 2
  if not m_param1:
      return 0
  require (100000 * m_param1) - (2 * ext_call.return_data[0] * m_param1) / m_param1 == -(2 * ext_call.return_data[0]) + 100000
  return ((100000 * m_param1) - (2 * ext_call.return_data[0] * m_param1) / 100000)
...
def unknownc2853b66(addr m_param1): # not payable
  require calldata.size - 4 >= 32
  require ext_code.size(msettingsAddress)
  static call msettingsAddress.0xe2c2f4ac with:
          gas gas_remaining wei
         args munknown14c84f3am[addr(m_param1)m]
  if not ext_call.success:
      revert with ext_call.return_data[0 len return_data.size]
  require return_data.size >= 32
